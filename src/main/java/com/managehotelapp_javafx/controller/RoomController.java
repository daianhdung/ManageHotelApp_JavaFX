...
public class RoomController implements Initializable {
    ...
    private  List<RoomEntity> availableRooms = new RoomRepositoryImp().getAvailableRooms();
    private  List<RoomEntity> unavailableRooms = new RoomRepositoryImp().getUnavailableRooms();
    RoomDetailController roomDetailController = new RoomDetailController();
    @Override
    public void initialize(URL url, ResourceBundle resourceBundle) {
        roomDetailController.roomController = this;
        getRoomItem("A001","Available","2 bed");
        getRoomItem("A002","Unavailable","2 bed");
...
//        }
        contextMenu.minWidthProperty().bind(tfSearch.widthProperty());
        contextMenu.maxWidthProperty().bind(tfSearch.widthProperty());
        tfSearch.setOnMouseClicked(event -> {
                contextMenu.getItems().clear();
                for (String suggestion : suggestions) {
                        MenuItem item = new MenuItem("Filter: " + suggestion);
                        item.setOnAction(e -> {
                                    ...
                                    if(suggestion=="Available"){
                                        ...
                                        {
                                            getRoomItem(r.getRoomNumber(),r.getRoomStatus().getTitle(),r.getRoomType().getDescription());
                                        }
                                    }else{
                                        for (var r : unavailableRooms)
                                        {
                                            getRoomItem(r.getRoomNumber(),r.getRoomStatus().getTitle(),r.getRoomType().getDescription());
                                        }
                                    }
        ...
        tfSearch.setOnKeyReleased(event -> {
            ...
            if (text.isEmpty()) {
                ...
                if (!contextMenu.getItems().isEmpty()) {
                    contextMenu.show(tfSearch, Side.BOTTOM, tfSearch.getWidth(), 0);
                } else {
                    contextMenu.hide();
                }
            }
    ...
    public void setBtnCheckin(Event event) throws IOException{
        RoomDTO rooomDTO = RoomDTO.getInstance();
        rooomDTO.setRoomList(seletedRooms);
        primaryStage = (Stage) ((Node)event.getSource()).getScene().getWindow();
        fxmlLoader = FXMLLoaderConstant.getRoomDetailScene();
        fxmlLoader.setController(roomDetailController);
        primaryStage.setUserData(seletedRooms);
        primaryStage.setScene(new Scene(fxmlLoader.load()));
        primaryStage.show();
    }
    private void getRoomItem(String RoomName, String RoomStatus, String RoomType){
        ...
        cbSelected.setOnAction(event -> {
            if(Objects.equals(RoomStatus, "Available")) {
                ...
                Label r = (Label) selectedItem.getScene().lookup("#lblRoomName_" + RoomName);
                if (cbSelected.isSelected()) {
                    seletedRooms.add(r.getText());
                } else {
                    Iterator<String> iterator = seletedRooms.iterator();
                    while (iterator.hasNext()) {
                        if (iterator.next().equals(r.getText())) {
                            iterator.remove();
                        }
                    }
                }
        ...
        gridPane.setOnMouseClicked(event -> {
            if(event.getClickCount() == 2) {
                ...
                var selectedItem = (Node) event.getSource();
                Label r = (Label) selectedItem.lookup("#lblRoomName_" + RoomName);
                if (Objects.equals(RoomStatus, "Available")) {
                    seletedRooms.add(r.getText());
        ...
        int c = gridPane2.getColumnCount();
        int r = gridPane2.getRowCount();
        int x = c == 0 ? 0 : c == 4 ? 0 : c , y = r == 0 ? 0 : c == 4 ? r : r-1;

        gridPane2.add(gridPane,x,y);
